...
func CommandCopy(cmd *COPY)(string, error){
	...
	if textPath && textDestino {
		return "ERROR: no se puede copiar un archivo a otro archivo, el destino debe ser una carpeta", errors.New("no se puede mover un archivo a otro archivo")
	}

	if CarpetaPath && textDestino {
		return "ERROR: no se puede copiar una carpeta a un archivo", errors.New("no se puede mover una carpeta a un archivo")
	}
				...
				if content.B_inodo != -1 && content.B_inodo != 0 && strings.Trim(string(content.B_name[:]), "\x00") != "." && strings.Trim(string(content.B_name[:]), "\x00") != ".."{
					...
					for i:=0; i<len(parenDirsPath); i++{
						...
						if strings.Trim(string(content.B_name[:]), "\x00") == parenDirsPath[i] {
							...
							err := copyRecursiva(inode, partitionSuperblock, partitionPath, parenDirsPath, destiDirPath, parenDirsDestino, destiDirDestino, textPath)
							if err != nil {
...
func copyRecursiva(inode *structures.Inode, partitionSuperblock *structures.SuperBlock, partitionPath string, parentDirs []string, destDir string, parentDirsDestino []string, destiDirDestino string, textPath bool)(error){
				...
				if content.B_inodo != -1 && content.B_inodo != 0 && strings.Trim(string(content.B_name[:]), "\x00") != "." && strings.Trim(string(content.B_name[:]), "\x00") != ".."{
					...
					for i := 0; i<len(parentDirs); i++{
						...
						if strings.Trim(string(content.B_name[:]), "\x00") == parentDirs[i] {
							...
							copyRecursiva(inode, partitionSuperblock, partitionPath, parentDirs, destDir, parentDirsDestino, destiDirDestino, textPath)
							return nil
						}
					}
					if strings.Trim(string(content.B_name[:]), "\x00") == destDir {
						...
						err = CopyDestino(inode1, partitionSuperblock, partitionPath, parentDirsDestino, destiDirDestino, refInodo, textPath, nombre)
						if err != nil {
							return err
						}
...
func CopyDestino(inode *structures.Inode, partitionSuperblock *structures.SuperBlock, partitionPath string, parentDirs []string, destDir string, refCambiar int32, textPath bool, nombre string)(error){
				...
				if content.B_inodo != -1 && content.B_inodo != 0 && strings.Trim(string(content.B_name[:]), "\x00") != "." && strings.Trim(string(content.B_name[:]), "\x00") != ".."{
					...
					for i := 0; i<len(parentDirs); i++{
						...
						if strings.Trim(string(content.B_name[:]), "\x00") == parentDirs[i] {
							...
							//llamar a la funcion recursiva
							err = CopyDestino(inode, partitionSuperblock, partitionPath, parentDirs, destDir, refCambiar, textPath, nombre)
							if err != nil {
								return err
							}
						}
