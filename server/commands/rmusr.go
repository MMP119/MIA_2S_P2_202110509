...
func ParseRmusr(tokens []string)(*RMUSR, string, error){
	...
	re := regexp.MustCompile(`(?i)-user="[^"]+"|(?i)-user=[^\s]+`)
...
func CommandRmurs(cmd *RMUSR) (string, error) {
		...
		for _, contenido := range folderBlock.B_content {
			...
			if name == "users.txt" {
				...
				if inode.I_block[0] == 1 {
					...
					if !groupFound {
						return "Error: usuario no encontrado", nil
					}
					...
					fmt.Println("-----------BORRAR USUARIO---------------")
					fileBlock.Print()
					fmt.Println("---------------------------")
					return "Comando RMUSR: realizado correctamente, Usuario eliminado con éxito", nil
				}
			}
		}
	}
