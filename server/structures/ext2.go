...
func (sb *SuperBlock) createFileInInodeExt2(path string, inodeIndex int32, parentsDir []string, destFile string, fileSize int, fileContent []string) error {
	// Crear un nuevo inodo
	inode := &Inode{}
	// Deserializar el inodo
	err := inode.Deserialize(path, int64(sb.S_inode_start+(inodeIndex*sb.S_inode_size)))
		...
		for indexContent := 2; indexContent < len(block.B_content); indexContent++ {
			...
			if len(parentsDir) != 0 {
				...
				if strings.EqualFold(contentName, parentDirName) {
					...
					// Si son las mismas, entonces entramos al inodo que apunta el bloque
					err := sb.createFileInInodeExt2(path, content.B_inodo, utils.RemoveElement(parentsDir, 0), destFile, fileSize, fileContent)
					if err != nil {
						return err
					}
					return nil
